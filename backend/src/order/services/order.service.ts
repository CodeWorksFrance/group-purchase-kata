...
export class OrderService {
  createBill(createBill: OrderInterface): BillInterface {
    ...
    bill.total =
      bill.amount +
      bill.items.reduce((acc, current) => (acc += current.fee), 0);
    return bill;
  }

  private getAmountByUser(
    createBill: OrderInterface,
  ): { name: string; amount: number; fee: number }[] {
    ...
    createBill.items.forEach((item) => {
      ...
      if (items.find((billItem) => billItem.name === item.buyer.name)) {
        items.find((billItem) => billItem.name === item.buyer.name).amount +=
          price;
      } else {
